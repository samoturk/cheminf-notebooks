...
def anymol2vina(mols, conf, rec, gen3d):
    ...
    for mol in mols:
        if str(mol.title)+".pdbqt" not in listdir('.'):
            ...
            molread = next(pybel.readfile("pdbqt", filename=str(mol.title)+".pdbqt"))
            molread.addh()
            ...
            if mol1 != mol2:
                ...
                mol.write("pdbqt", str(mol.title)+".pdbqt", overwrite=True)
                try:
                    call(["vina", "--config", conf, "--receptor", rec, "--ligand", str(mol.title)+".pdbqt", "--out",
                          str(mol.title)+"_docked.pdbqt"])
                except:
                    print("Error with: " + str(mol.title))
            else:
...
if __name__ == "__main__":
    ...
    if len(argv) == 5:
        ...
        mols = pybel.readfile("%s" % ftypein, filename="%s" % molsin)
        conf = argv[2]
        rec = argv[3]
